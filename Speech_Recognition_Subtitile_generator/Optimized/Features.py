...
def Features(mouth_centroid_x, mouth_centroid_y, b, shotname, Gabor_path, SheetPath, FeaturesPath):
    create_directory(SheetPath)
    create_directory(FeaturesPath)

    cur_dir = os.path.join(SheetPath, shotname)
    create_directory(cur_dir)

    cur_dir2 = os.path.join(FeaturesPath, shotname)
    create_directory(cur_dir2)

    image = cv2.imread(Gabor_path, 0)
    thresh = filters.threshold_yen(image)
    bwimg = (image >= thresh)
    labels, num = measure.label(bwimg, return_num=True, background=1)
    image_label_overlay = color.label2rgb(labels, image=image, bg_label=-1)
    ...
    if num > 0:
        ...
        ax.add_patch(cir1)
        ax.add_patch(rect)
        cur_dir2 = os.path.join(cur_dir2, '')  # Save path

        if not os.path.exists(cur_dir2):
            os.makedirs(cur_dir2)

        Features_path = os.path.join(cur_dir2, f'{b:02d}.png')
        plt.savefig(Features_path)
        plt.close('all')
        ...
        parameters = ['Box_width', 'Box_height', 'Final_area', 'Centroid_x', 'Centroid_y', 'intensity', 'orientation']
        value = [width, height, Final_area, Final_centroidx, Final_centroidy,
                 Final_meanintensity * Final_area, Final_orientation]

        cur_dir = os.path.join(cur_dir, '')  # Save path

        if not os.path.exists(cur_dir):
            os.makedirs(cur_dir)

        sheetPath = os.path.join(cur_dir, f'{b:02d}.csv')

        with open(sheetPath, 'w', newline='') as f:
            writer = csv.writer(f)
            for param, val in zip(parameters, value):
...
# To optimize the given code without changing variable names, you can make the following improvements:

# Import necessary modules: It seems that the code is missing the import statements for required modules like os, cv2, numpy, matplotlib.pyplot, matplotlib.patches, skimage.filters, skimage.measure, and skimage.color. Make sure to include these import statements at the beginning of your code.

# Remove unused variables and comments: Several commented lines are unnecessary and can be removed from the code to improve readability.

# Combine similar code blocks: The code blocks for creating directories can be combined into a single function, which can be reused for both SheetPath and FeaturesPath directories.

# Use context managers for file operations: Instead of explicitly opening and closing files using open() and close() functions, use context managers (with statement) for better resource management. This applies to both image and CSV file operations.

# Optimize loop logic: Instead of initializing minw and minh with high values like 1000, you can initialize them with float('inf') to represent infinity. This way, you can avoid using arbitrary high values and simplify the loop logic.
